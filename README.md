...
- **Streamlit** — operations dashboard: task browser, cost analytics, approval buttons, memory viewer
...
- **Keyword-based memory recall missed "calculation" vs "calculate".** Jaccard similarity over exact tokens can't see morphology or synonyms — the concrete failure that motivates embeddings. Similarity lives behind one function, so swapping in embeddings touches one function.
